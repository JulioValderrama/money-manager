...
const index = async (_req: Request, res: Response) => {
  try {
    ...
    if (result.length == 0) {
      throw new Error('No results FOUND in CATEGORY-TYPE table');
    }
...
const show = async (req: Request, res: Response) => {
  try {
    ...
    if (result == undefined) {
      throw new Error(`No results FOUND for CATEGORY-TYPE with id = ${req.params.id}`);
    }
    res.status(200).json(result);
  } catch (error) {
    const formatedError = formatError(error);
...
const destroy = async (req: Request, res: Response) => {
  try {
    ...
    if (result == undefined) {
      throw new Error(`No CATEGORY-TYPE found with id = ${req.params.id}`);
    }
    res.status(200).json(result);
  } catch (error) {
    const formatedError = formatError(error);
